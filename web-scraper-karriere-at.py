...
fields0 = ["Finanzen, Bankwesen", "Führung, Management"]#["Informatik", "Assistenz, Verwaltung", "Beratung, Consulting", "Coaching, Training", "Einkauf, Logistik", "Finanzen, Bankwesen", "Führung, Management"]
fields1 = ["Produktion, Handwerk", "Sonstige Berufe"]#["Gastronomie, Tourismus", "Grafik, Design", "IT, EDV", "Marketing, PR", "Personalwesen", "Pharma, Gesundheit, Soziales", "Produktion, Handwerk"]
fields2 = ["Verkauf, Kundenbetreuung", "Wissenschaft, Forschung"]#["Rechnungswesen, Controlling", "Rechtswesen", "Sachbearbeitung", "Sonstige Berufe", "Technik, Ingenieurwesen", "Verkauf, Kundenbetreuung", "Wissenschaft, Forschung"]
...
def checkNumsForSalary(gehaelter):
    gehalt = 0.0
    #check if a number can be a salary
    for num_val in gehaelter:
        if (num_val != ""):
            num_val = int(num_val)
            if(num_val>1000 and num_val<250000 and isNoPlz(num_val, postleitzahlen) and str(num_val).startswith('0') == False and currentYear != num_val):
                gehalt = float(num_val);
                if(gehalt>10000):
                        gehalt=gehalt/12
    return gehalt
...
def extract_job_info(driver, job_url, csv_str, valid_file_name):
    ...
    #Find content, replace commas to avoid errors when creating the CSV file
    titel = driver.find_element_by_tag_name('h1').text.replace(",", ".")
    ...
    for gehaelter in gehalt_list:
        ...
        if(gehalt == 0.0):
            ...
            gehalt = checkNumsForSalary(gehaelter)
        else:
            break

...
#"" will search for everything
#leave an array empty if you don't want to use that thread
...
#Liste von Bundesländern. Einzelne Bezirke sind auch möglich.
...
def extract_job_info(driver, job_url, csv_str, valid_file_name):
    ...
    try:
        metaItems = driver.find_elements_by_class_name("m-jobHeader__metaItem")#list index out of range (wenn der Job nicht mehr verfügbar ist)
...
def get_jobs_info(field, bundesland):
    ...
    #if all jobs are searched for ("", see comments on top), replace "jobs//" in the url with "jobs/"
    if (field == ""):
